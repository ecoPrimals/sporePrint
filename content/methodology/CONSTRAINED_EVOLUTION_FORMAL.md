...
The computational analogies in this paper are grounded in real microbiology. The author holds a BS in Microbiology from Michigan State University (Microbial Genetics, Prokaryotic Cell Physiology, Medical Microbiology, Virology, Molecular Pathogenesis), with bench experience in high-throughput sequencing (Sandia National Laboratories), fermentation and microbial analysis (MSU Bioeconomy Institute), and bacterial isolation and sequencing via the Ribosomal Database Project (ROME Lab, MSU). The constrained evolution methodology described in this paper is not an analogy borrowed from biology for rhetorical convenience. It is a model informed by years of working with microbial populations under selective pressure.
...
This is not a computational side-project. It is traditional science: culture the frozen stocks, extract DNA, perform whole-genome sequencing across timepoints and populations, and apply modern bioinformatics and statistical methods (informed by the author's MS in Data Science - computational optimization, machine learning, deep learning, applied statistics) to the resulting dataset. The LTEE library is at MSU. The sequencing infrastructure exists. The analytical framework is described in this paper.
