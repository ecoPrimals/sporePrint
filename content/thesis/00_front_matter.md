...
**Computational system.** The principle was applied to construct the ecoPrimals ecosystem: {{ total_stat(stat="total_loc_display") }} lines of Rust across {{ total_stat(stat="total_primals") }} primals, {{ total_stat(stat="wgsl_files") }} WGSL shaders, and {{ total_stat(stat="total_tests_display") }} tests, built by a single developer with AI assistance over approximately 10 months. The Rust type system served as the environmental constraint (analogous to temperature in hot springs), the Pure Rust directive and capability-based architecture served as selective direction (analogous to nutrient limitation in the LTEE), and AI-assisted code generation served as the mutation operator (analogous to DNA replication with error). The compile-time fitness check eliminates unsound solutions before they reach the binary — analogous to a ribosome that refuses to translate lethal mRNA.
