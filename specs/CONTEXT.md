...
- **WAN mesh status (Wave 111)** — **FEDERATION WORKING.** Depot rebuilt (2026-06-12T12:37Z). songbird v0.2.1 fetched (BLAKE3: `3fc94365...` ✓). Wire fix confirmed: `federation.status` reports `enabled: true`. Auto-reconnect peer health loop measures 64ms RTT to VPS (157.230.3.183:7700). `mesh.init` succeeds, peer reachable via direct path. Persistent relay connections (`active_connections > 0`) pending VPS-side rebuild to `fe47c012` (Wave 113).
