...
- **`spore-validate` v0.3.1** — 33-module Rust crate (10,718L): typed validation, link checking, notebook rendering, metric sync, graph building, certification, CAS manifest + push, discovery, NUCLEUS profile validation + display, depot integrity verification, petalTongue IPC client, Tower P1 readiness probe (TOML-driven), HTTP/tar utilities, trait-based VCS, shared IPC module, parity integration tests. 279 tests (247 unit + 29 integration + 3 refresh_write, 6 parity ignored), `#![forbid(unsafe_code)]`, `#![warn(missing_docs)]`, zero clippy warnings (pedantic + nursery), zero C toolchain dependencies (blake3 pure-Rust, flate2 miniz_oxide), `cargo-deny` clean, `toml` 1.x (TOML spec 1.1). Module architecture: `main.rs` (thin entry, 66L) → `cli.rs` (Clap types) → `dispatch.rs` (command routing) → `commands*.rs` (validate/provenance/discover). `commands.rs` (785L) split into 4 focused modules; `main.rs` (687L) split into `cli.rs` + `dispatch.rs`. Tower probes data-driven via `default_tower_probes.toml` (embedded, `include_str!`). All files under 620L (max: content.rs at 616). CAS push error handling evolved to structured `PushFileOutcome::Error(String)`. Fetch returns `Result<Vec<FetchOutcome>, Error>` (was silent `Vec::new()` on failure).
...
├── crates/spore-validate/   # Rust validation crate (33 modules, 272 tests)
