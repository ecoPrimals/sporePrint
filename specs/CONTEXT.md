...
## Current State (July 12, 2026 — Wave 137b)

- **270 published pages** across 18 sections (Architecture, Audience, Contact, Glossary, guideStone, Lab, Methodology, reachOut, Philosophy, Primals, Products, Science, Site Index, Sitemap, Springs, Story, Technical, Thesis) + landing + lab notebooks
...
- **Accessibility: WCAG 2.2 AAA target** — AA contrast ratios achieved (4.8:1+ accent), `prefers-reduced-motion` support, `prefers-contrast: more` (light + dark), `forced-colors: active` (Windows High Contrast), ARIA landmarks/combobox/listbox, decorative emoji hidden, keyboard navigation, semantic heading hierarchy, text-only status indicators, mobile TOC, 404 with full chrome. **AI accessibility**: all 23 navigational tables across 9 section indexes converted to ordered/unordered lists (tables strip links in AI fetch-to-text tools). JSON-LD `hasPart` on all 12 sections with child pages (generic `CollectionPage` fallback + `ScholarlyArticle` for science). **Progressive richness**: 5-level machine-readable topology (Billboard → Overview → Catalog → Topology → Reinforcement). Per-page JSON-LD on all 270 pages (`Article`/`ScholarlyArticle` with typed author). 28 key pages carry typed `relatedLink` companions across domains + visible companion panels. `llms.txt` content topology section. Screen reader testing is evolution target. petalTongue will set the universal access standard — any human capability profile, no intermediary.
...
├── crates/spore-validate/   # Rust validation crate (34 modules, 287 tests)
...
Audience, Methodology, reachOut, and Technical are accessible via the site tree sidebar.
